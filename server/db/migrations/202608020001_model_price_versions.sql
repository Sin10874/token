...
INSERT INTO model_price_versions (
  ...
  (
    ...
    0.95, 4, 0.19, NULL,
    'hit_miss', 262144,
    'https://platform.kimi.ai/docs/pricing/chat', '2026-08-02'
  ),
  (
...
INSERT OR IGNORE INTO model_price_backfills (
  ...
  previous_input_cost, previous_output_cost,
  previous_cache_read_cost, previous_cache_write_cost, previous_total_cost,
  backfilled_at
)
SELECT
  ue.id, '202608020001_model_price_versions',
  ue.input_cost, ue.output_cost,
  ue.cache_read_cost, ue.cache_write_cost, ue.total_cost,
  CAST(strftime('%s', 'now') AS INTEGER) * 1000
FROM usage_events AS ue
JOIN model_price_versions AS price
  ON price.model_id = ue.model
  AND ue.timestamp_ms >= price.valid_from_ms
  AND (price.valid_to_ms IS NULL OR ue.timestamp_ms < price.valid_to_ms)
WHERE ue.total_cost = 0
  AND NOT (price.cache_semantics = 'hit_miss' AND ue.cache_write_tokens <> 0);

UPDATE usage_events AS ue
SET
  input_cost = (ue.input_tokens * price.input_price) / price.per_tokens,
  output_cost = (ue.output_tokens * price.output_price) / price.per_tokens,
  cache_read_cost = (ue.cache_read_tokens * price.cache_read_price) / price.per_tokens,
  cache_write_cost = (ue.cache_write_tokens * COALESCE(price.cache_write_price, 0)) / price.per_tokens,
  total_cost = (
    (ue.input_tokens * price.input_price)
    + (ue.output_tokens * price.output_price)
    + (ue.cache_read_tokens * price.cache_read_price)
    + (ue.cache_write_tokens * COALESCE(price.cache_write_price, 0))
  ) / price.per_tokens
FROM model_price_versions AS price, model_price_backfills AS backfill
WHERE backfill.event_id = ue.id
  AND backfill.migration_id = '202608020001_model_price_versions'
  AND price.model_id = ue.model
  AND ue.timestamp_ms >= price.valid_from_ms
  AND (price.valid_to_ms IS NULL OR ue.timestamp_ms < price.valid_to_ms)
  AND ue.total_cost = backfill.previous_total_cost;
