...
BEGIN
  IF target_oid IS NULL THEN
    RAISE EXCEPTION 'tokend_upload_events(text,jsonb,jsonb) is missing after rollback';
  END IF;
  ...
  IF target_owner <> 'postgres' THEN
    RAISE EXCEPTION 'tokend_upload_events(text,jsonb,jsonb) has unexpected owner after rollback';
  END IF;
  IF target_security_definer IS DISTINCT FROM TRUE THEN
    RAISE EXCEPTION 'tokend_upload_events(text,jsonb,jsonb) must remain SECURITY DEFINER after rollback';
  END IF;
  IF target_config IS DISTINCT FROM ARRAY['search_path=public, pg_temp']::TEXT[] THEN
    RAISE EXCEPTION 'tokend_upload_events(text,jsonb,jsonb) search_path is not pinned after rollback';
  END IF;
END
$$;
